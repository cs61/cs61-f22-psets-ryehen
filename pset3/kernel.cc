...
#define HZ 1                  // timer interrupt frequency (interrupts/sec)
...
uintptr_t syscall(regstate* regs) {
    ...
    //if (regs->reg_rax == 6 || regs->reg_rax == 5) {
        log_printf("syscall: %i\n", regs->reg_rax);
    //}
    // If Control-C was typed, exit the virtual machine.
    check_keyboard();
...
int syscall_fork() {
    ...
    for (; srcit.va() < MEMSIZE_VIRTUAL; srcit += PAGESIZE, dstit += PAGESIZE) {
        int r = 0;
        if (srcit.va() < PROC_START_ADDR || srcit.va() == CONSOLE_ADDR) {
            r = dstit.try_map(srcit.pa(), srcit.perm());
        } else if (srcit.perm() & PTE_U && srcit.perm() & PTE_W) {
    ...
    if (failure) {
        log_printf("V1!\n");
        dstit = vmiter(ptable[pid].pagetable, 0);
        for (; dstit.va() < srcit.va(); dstit += PAGESIZE) {
            kfree((void*) dstit.pa());
        }

        
        log_printf("%p\n", ptable[pid].pagetable);
        for (ptiter ptit(ptable[pid].pagetable); !ptit.done(); ptit.next()) {
            kfree((void*) ptit.kptr());
        }

        log_printf("callee PID : %i\n", pid);
        log_printf("caller PID : %i\n", current->pid);
...
void syscall_exit() {
    ...
    for (; it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
        if ((uintptr_t) it.pa() != (uintptr_t) -1) {
            kfree((void*) it.pa());
        }
    }
    ...
    for (; ptit.va() < MEMSIZE_VIRTUAL; ptit.next()) {
        kfree((void*) ptit.kptr());
    }
